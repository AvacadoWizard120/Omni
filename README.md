...
- Standalone Java 8 builds produce LiteLoader `.litemod` files
...
./gradlew :1.8.9-liteloader:build
...
- LiteLoader uses `litemod.json` and a LaunchWrapper class transformer. Each
  target produces a drop-in `.litemod`.
...
- LiteLoader 1.5.2 and 1.6.2: a normal drop-in `.litemod` cannot register
  Omni's transformer.
- Other LiteLoader patch versions and Minecraft 1.13+: no official LiteLoader
  build.
